...
struct SHEET *open_console(struct SHEET *shtctl, unsigned int memtotal);
	...
	*((int *) 0x0fec) = (int) &fifo;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			i = fifo32_get(&fifo);
			io_sti();
			if (key_win != 0 && key_win->flags == 0) {	/* ?E?B???h?E???????? */
				if (shtctl->top == 1) {
					key_win = 0;
				} else {
			...
			if (256 <= i && i <= 511) { /* ?L?[?{?[?h?f?[?^ */
				...
				if ('A' <= s[0] && s[0] <= 'Z') {	/* ??????????A???t?@?x?b?g */
					if (((key_leds & 4) == 0 && key_shift == 0) ||
							((key_leds & 4) != 0 && key_shift != 0)) {
						s[0] += 0x20;	/* ?啶????????????? */
					}
				}
				if (s[0] != 0 && key_win != 0) { /* ??????A?o?b?N?X?y?[?X?AEnter */
